...
public class InventoryScreen extends AbstractContainerScreen<InventoryMenu> {
    ...
    private static final ResourceLocation PANTIES_SLOT_LOCATION = new ResourceLocation(TartarusMod.MOD_ID, "textures/gui/container/panties.png");
    ...
    protected void renderBg(GuiGraphics pGuiGraphics, float pPartialTicks, int pX, int pY) {
        ...
        Minecraft.getInstance().player.getCapability(ClothesProvider.CLOTHES_INVENTORY).ifPresent(inv->{
            if(inv.getInventory().getStackInSlot(0).isEmpty()) {
                pGuiGraphics.blit(HAT_SLOT_LOCATION, relX + 80, relY + 8, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(1).isEmpty()) {
                pGuiGraphics.blit(SHIRT_SLOT_LOCATION, relX + 80, relY + 26, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(2).isEmpty()) {
                pGuiGraphics.blit(PANTS_LOCATION, relX + 80, relY + 44, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(3).isEmpty()) {
                pGuiGraphics.blit(SHOES_SLOT_LOCATION, relX + 80, relY + 62, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(4).isEmpty()) {
                pGuiGraphics.blit(BRA_SLOT_LOCATION, relX + 98, relY + 26, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(5).isEmpty()) {
                pGuiGraphics.blit(JACKET_SLOT_LOCATION, relX + 116, relY + 26, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(6).isEmpty()) {
                pGuiGraphics.blit(GLOVES_SLOT_LOCATION, relX + 134, relY + 26, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(7).isEmpty()) {
                pGuiGraphics.blit(PANTIES_SLOT_LOCATION, relX + 152, relY + 26, 0, 0, 16, 16, 16, 16);
            }
            if(inv.getInventory().getStackInSlot(8).isEmpty()) {
                pGuiGraphics.blit(BELT_SLOT_LOCATION, relX + 98, relY + 44, 0, 0, 16, 16, 16, 16);
            }
        });
